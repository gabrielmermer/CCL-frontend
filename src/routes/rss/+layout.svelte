...
import plusActive from '$lib/images/plus-active.svg';
import plusDisabled from '$lib/images/plus-disabled.svg';
import minus from '$lib/images/minus.svg';
import minusActive from '$lib/images/minus-active.svg';
import minusDisabled from '$lib/images/minus-disabled.svg';
...
<div class="fixed top-0 left-0 h-screen w-[475px] m-0 bg-gradient-to-br from-[#F3F3F3]/90 to-white/10 z-20">
	...
	<div class="flex items-center">
		...
		{#if pageUrlFormated == "add"}
			<a href="/rss" class="py-2"  on:click={toggle_isRemoving}>
				<img draggable="false" src="{minusDisabled}" alt="" class="ml-9 mt-[39px]">
			</a>
		{:else}
			
			{#if isRemoving == true }
				<a href="#" class="py-2"  on:click={toggle_isRemoving}>
					<img draggable="false" src="{minusActive}" alt="" class="ml-9 mt-[39px]">
				</a>
			{:else}
	...
	{#if pageUrlFormated == "add"}
		...
		{#each data.feeds as feed}
			...
			{#if pageUrlFormated == feed.feed_name}
				...
				{/if}
			{:else}
			<a fade href="/rss/{feed.feed_name}" class="ml-2 font-supply text-2xl text-beige10"> &nbsp&nbsp{feed.feed_name}</a> <br>
			{/if}
			</div>
		{/each}
	...
	{:else}
		...
		{#each data.feeds as feed}
			...
			{#if pageUrlFormated == feed.feed_name}
				{#if isRemoving == true }
					<a href="/rss/{feed.feed_name}" class="ml-2 font-supply text-2xl text-black hover:text-green-300">&nbsp&nbsp{feed.feed_name}</a> <br>
				{:else}
					<a href="/rss/{feed.feed_name}" class="ml-2 font-supply text-2xl text-black hover:text-green-500">> {feed.feed_name}</a> <br>
				{/if}
			{:else}
			<a fade href="/rss/{feed.feed_name}" class="ml-2 font-supply text-2xl text-beige80 hover:text-green-400"> &nbsp&nbsp{feed.feed_name}</a> <br>
			{/if}
			</div>
		{/each}
...
<img draggable="false" src="{blobPink3}" alt="" class="mt-10 ml-10 fixed bottom-0 right-0 -z-20">

<p class="select-none fixed right-[-630px] bottom-[415px] -rotate-90 text-[150px] font-supply bg-clip-text text-transparent bg-gradient-to-b from-white/90 to-white/20">00110100011001</p>
